...
class EnemyT :public IGameObject
{
...
public:
	...
	//登場済みの敵に突撃させるメソッド、sはスピード、tは突撃までの猶予フレーム数、fは突撃後の猶予を正の整数で
	void Totusiro(float s, float t,float f)
	{
		M_TOTUC = t;
		m_totuc = t;
		m_totusoku = s;
...
private:
	...
	//m_totuc初期値
	int M_TOTUC = 60;
	//突撃行動パターン終了フレームを記録する変数
	int m_totufin;
	//1フレームごとに移動する距離を記録する変数、このクラスでは敵をプレイヤーの方向に向かせるために使用
	Vector3 m_hureichi;
	...
	//発射アニメに入ってから発射するまで
	const int HASSYAMADE = -30;
	//発射してからアニメが終わるまで
	const int HASSYAEND = -99;
	...
	Vector3 m_size;
	//コリジョンのサイズ
	const float COLXZ = 20.0f;
	const float COLY = 30.0f;
